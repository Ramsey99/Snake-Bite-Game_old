...
const Level11 = ({ onNext }) => {
  const initialDeck = new Map([
    ['one', '20 WBCT'],
    ['two', 'Every hour'],
    ['three', 'TIll next 4 hours'],
    ['four', '10 WBCT'],
    ['five', 'INR'],
    ['six', 'Every 4 hours'],
    ['seven', 'Till next 24 hours']
  ]);

  const correctSequence = ['one', 'two', 'three']; // Correct sequence for Level 9
  const [deck, setDeck] = useState([]);
  const [result, setResult] = useState([]);
  const [selectedCards, setSelectedCards] = useState([{}, {}, {}]); // 3 slots for selection
  ...
  const handleCardClick = (card) => {
    if (result.length >= 3 || result.includes(card[0])) return; // Prevent selecting more than 3 cards or the same card twice

    const updatedResult = [...result, card[0]];
    setResult(updatedResult);

    const newSelectedCards = [...selectedCards];
    newSelectedCards[result.length] = { text: card[1], key: card[0] }; // Store the selected card in the correct slot
  ...
  const resetGame = () => {
    setResult([]);
    setSelectedCards([{}, {}, {}]); // Reset to 3 empty slots
  ...
  return (
    <>
      ...
      <div className="text-xl mb-8">
        <h2 className="text-center text-lg font-bold mb-4">Selected Cards</h2>
