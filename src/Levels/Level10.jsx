...
const Level10 = ({ onNext }) => {
  const initialDeck = new Map([
    ['one', 'AN maintenance dose'],
    ['two', 'Wait 30 min for improvement'],
    ['three', 'AN loading dose'],
    ['four', 'Wait for 1 hour'],
    ['five', 'Transfer to referral hospital']
  ]);

  const correctSequence = ['one', 'two']; // Correct sequence for Level 10
  const [deck, setDeck] = useState([]);
  const [result, setResult] = useState([]);
  const [selectedCards, setSelectedCards] = useState([{}, {}]); // 2 slots for selection
