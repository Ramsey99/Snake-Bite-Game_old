...
const Level3 = ({ onNext }) => {
  const initialDeck = new Map([
    ['one', 'Secure Respiratory and Airway'],
    ['two', 'Admission'],
    ['three', 'Start IVF with NS/5D'],
    ['four', 'Inj. TT'],
    ['five', 'Secure Airway'],
    ['six', 'Avoid I/V cannula insertion in case of Haemotoxic bite'],
    ['seven', 'Avoid any IM or IV injections'],
    ['eight', 'Start Antibiotics immediately'],
    ['nine', 'Sedate with Diazepam'],
  ]);

  const correctSequence = ['one', 'two', 'three', 'four']; // Correct sequence for Level 3
  ...
  const handleCardClick = (card) => {
    if (result.length >= 4 || result.includes(card[0])) return; // Prevent selecting more than 4 cards or the same card twice
